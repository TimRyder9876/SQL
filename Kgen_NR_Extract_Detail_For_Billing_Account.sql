...
(select
  ...
  select  
   ...
   m.trans_id,
   m.activity_type|| 
   case when trim(m.activity_sub_type) is null then '' else 
    '-'||m.activity_sub_type end ||':'|| m.acnt_type || 
    case when trim(m.charging_type) = 'LFC' then ':LFC' 
         when trim(m.charging_type) = 'RCK' then ':RCK' 
         when trim(m.product_type) is null then '' 
         when trim(m.product_type) in ('B','M','W','Y') then ':'||'Print' 
         when trim(m.product_type) in ('V') then ':'||'Digital' 
         else ':'|| m.product_type end as psft_je_line_description, 
   m.account_id,
   m.product_code,
   m.prod_issue_num,
   m.logical_acnt,
